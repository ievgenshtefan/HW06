...
def generate_video_files(path):
    ...
    with open(path / f"{get_random_filename()}.{choice(videos).lower()}", "wb") as f:
        f.write(MESSAGE.encode())
...
def generate_random_files(path):
    for i in range(3, randint(5, 7)):
        function_list = [generate_text_files, generate_music_files, generate_video_files, generate_archive_files, generate_image]
